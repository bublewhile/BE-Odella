...
<div class="card shadow-sm" style="max-width:640px">
    <div class="card-header bg-white fw-semibold">
        <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Promo
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.promo.update', $promo->id_promo) }}">
            @csrf @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="kode_promo" class="form-control @error('kode_promo') is-invalid @enderror"
                       value="{{ old('kode_promo', $promo->kode_promo) }}" style="text-transform:uppercase">
                ...
                <select name="id_produk" class="form-select">
                    ...
                    @foreach($produks as $p)
                        <option value="{{ $p->id_produk }}"
                            {{ old('id_produk', $promo->id_produk) == $p->id_produk ? 'selected' : '' }}>
                            {{ $p->nama_produk }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="number" name="persen_diskon" class="form-control @error('persen_diskon') is-invalid @enderror"
                           value="{{ old('persen_diskon', $promo->persen_diskon) }}" min="1" max="100" step="0.01">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    ...
                    <input type="datetime-local" name="tgl_berlaku" class="form-control"
                           value="{{ old('tgl_berlaku', $promo->tgl_berlaku->format('Y-m-d\TH:i')) }}">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="datetime-local" name="tgl_berakhir" class="form-control"
                           value="{{ old('tgl_berakhir', $promo->tgl_berakhir->format('Y-m-d\TH:i')) }}">
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="is_active" value="1"
                           id="isActive" {{ old('is_active', $promo->is_active) ? 'checked' : '' }}>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save me-1"></i>Update Promo
                </button>
